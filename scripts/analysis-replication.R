# This script contains regressions and other analysis supporting the key assertions made in the text of the article. The below is a very small set of the statistical analysis conducted overall, meant to illustrate our main results. This means that the various robustness checks and auxiliary analysis we conducted to increase our confidence in these results is for the most part not included here. Standard errors are clustered at the country level using the method detailed in Cameron, Gelbach, & Miller (2011), analysis is weighted by country population unless otherwise noted. We again stress, as we do in the article, that these are not directional relationships (i.e. causal relationships). We hope other researchers will find these results interesting and explore them further.
